...
- ~~How do we bootstrap the golden set? Do we start with synthetic examples, or do we capture real-world cases from early human-supervised agent operation?~~ Functional evals bootstrap with hand-crafted cases under `eval/`; see [ADR 0044](../ADRs/0044-functional-evals-for-agent-pipelines.md). Prompt-level evals and synthetic expansion remain open.
